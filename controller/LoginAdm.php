...
if (isset($dados)) {
    ...
    $verificaLogin->fullRead("SELECT * FROM tbl_admin WHERE email =:email", "email={$dados['email']}");
    ...
    if (!empty($resultadoverificaLogin)) {
        ...
        if (password_verify($dados['senha'], $resultadoverificaLogin[0]['senha']) == false) { // SE A SENHA DIGITADA FOR DIFERENTE DA QUE ESTÁ NO BD
            ...
            $_SESSION['adm'] = $resultadoverificaLogin[0]; //inciando sessão do usuario todos seus dados 
        }
